...
| `ElementsAreArray({ e0, e1, ..., en })`, `ElementsAreArray(macierz)`, or `ElementsAreArray(macierz, count)` | The same as `ElementsAre()` except that the expected element values/matchers come from an initializer list, vector, or C-style macierz. |
...
| `UnorderedElementsAreArray({ e0, e1, ..., en })`, `UnorderedElementsAreArray(macierz)`, or `UnorderedElementsAreArray(macierz, count)` | The same as `UnorderedElementsAre()` except that the expected element values/matchers come from an initializer list, vector, or C-style macierz. |
